...
def w2v_paraphrase(input_text, input_text_delex,const, isUser, turn_num, dial_id):
  ...
  if not random.randint(0,6) :
    return input_text, input_text_delex
    ...
    for i in to_augment:
      ...
      for j in output:
        ...
        if j.lower() != i and j.lower() != i+'s' and j.lower() != i+'es' and j.lower() not in punc and not j.lower().strip().isnumeric():
          ...
          if not isUser or (isUser and  m.check_bspan(dial = dial_id, turn_num = turn_num, augmentation = augmented.replace(i, j.lower().strip()))) and random.randint(0,4):
...
with open("./data/augmentations/w2v_replace_x5/data_for_damd_.json", "w") as f:
  json.dump(data_aug, f, indent=2)

for ru in ['1','2','3'] :

    logging.info('Reading data for '+ru)
    print('Reading data for '+ru)
