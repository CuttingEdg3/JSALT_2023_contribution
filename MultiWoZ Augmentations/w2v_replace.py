...
#os.chdir('/content/drive/MyDrive/PhD/UBAR/UBAR-MultiWOZ')
wv = KeyedVectors.load("/workspace/cvlachos/projects/UBAR/UBAR-MultiWOZ/data/augmentations/w2v_replace/vectors.bin", mmap='r')
...
def w2v_paraphrase(input_text, input_text_delex,const, isUser, turn_num, dial_id):
  ...
  try :
    ...
    for i in input_text_delex.split() :
      if i not in punc and not i[0].isnumeric() and i not in domains and i not in stop_words and not i.startswith('[') and i not in const and not i.isnumeric() and i not in ['am','pm']:
        to_augment.append(i)
    #print(to_augment)

    for i in to_augment:
      output = wv.most_similar(i)

      output = [' '.join(i.split('_')) for i,_ in output if i[0].islower()]
      ...
      for j in output:
        ...
        if j.lower() != i and j.lower() != i+'s' and j.lower() != i+'es' and j.lower() not in punc and not j.lower().strip().isnumeric():
          #print(j['token_str'].lower(), i)
          print()
          print('replacing',i,'with',j.lower().strip())
          print(augmented)

          if not isUser or (isUser and  m.check_bspan(dial = dial_id, turn_num = turn_num, augmentation = augmented.replace(i, j.lower().strip()))):

            augmented = augmented.replace(i, j.lower().strip())
            augmented_delex = augmented_delex.replace(i, j.lower().strip())
            break
...
with open("./data/augmentations/w2v_replace/data_for_damd.json", "w") as outfile:
    ...
    for n, i in enumerate(list(data.keys())[k:]):
      ...
      if i in data10 :
        ...
        for turn_num, turn in enumerate(data[i]['log']):
          ...
          turn['user'], turn['user_delex'] = w2v_paraphrase(turn['user'], turn['user_delex'] ,turn['cons_delex'], True, turn_num, i)
          print(turn['user'], turn['user_delex'])
          
          print()
          print(turn['resp_pre_delex'], turn['resp'])
          turn['resp_pre_delex'], turn['resp'] = w2v_paraphrase(turn['resp_pre_delex'], turn['resp'],turn['cons_delex'], False, turn_num,i )
          print(turn['resp_pre_delex'], turn['resp'])
        ...
        data_aug = json.loads(open('./data/augmentations/w2v_replace/data_for_damd.json', 'r', encoding='utf-8').read().lower())
        data_aug[i] = data[i]


        with open("./data/augmentations/w2v_replace/data_for_damd.json", "w") as f:
