...
def roberta_paraphrase(input_text, input_text_delex,const, isUser, turn_num, dial_id):
  ...
  try :
    ...
    for i in to_augment:
      #print(i)
      inputs = tokenizer(input_text.replace(i,'<mask>'), return_tensors="pt")
      #print(input_text)
      ...
      for j in output:
        ...
        if j['token_str'].lower().strip() != i and j['score'] > 0.05 and j['token_str'].lower().strip() not in punc and not j['token_str'].lower().strip().isnumeric():
...
#with open("./data/augmentations/roberta_replace/data_for_damd.json", "w") as outfile:
    ...
    for n, i in enumerate(list(data.keys())[k:]):
      ...
      if i in data10 :
        ...
        for turn_num, turn in enumerate(data[i]['log']):
          ...
          turn['user'], turn['user_delex'] = roberta_paraphrase(turn['user'], turn['user_delex'] ,turn['cons_delex'], True, turn_num, i)
          print(turn['user'], turn['user_delex'])
          
          print()
          print(turn['resp_pre_delex'], turn['resp'])
          turn['resp_pre_delex'], turn['resp'] = roberta_paraphrase(turn['resp_pre_delex'], turn['resp'],turn['cons_delex'], False, turn_num,i )
          print(turn['resp_pre_delex'], turn['resp'])
        ...
        data_aug = json.loads(open('./data/augmentations/roberta_replace/data_for_damd.json', 'r', encoding='utf-8').read().lower())
        data_aug[i] = data[i]


        with open("./data/augmentations/roberta_replace/data_for_damd.json", "w") as f:
