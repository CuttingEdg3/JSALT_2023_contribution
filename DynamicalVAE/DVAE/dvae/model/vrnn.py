...
class VRNN(nn.Module):
    ...
    def build(self):
        ...
        self.gen_out = nn.Sequential(
                        nn.Linear(dim_hz_x, self.y_dim))
                        
        self.gen_out_std = nn.Sequential(
                        nn.Linear(dim_hz_x, self.y_dim),
                        nn.Softplus())
    ...
    def generation_x_std(self, feature_zt, h_t):

        dec_input = torch.cat((feature_zt, h_t), 2)
        dec_output = self.mlp_hz_x(dec_input)
        y_t = self.gen_out_std(dec_output)
    ...
    def forward(self, x, inf = False):
        ...
        y_std = torch.zeros((seq_len, batch_size, self.y_dim)).to(self.device)
        ...
        for t in range(seq_len):
            ...
            h_list.append(h_t.squeeze(1).numpy())
            
        self.z_mean_p, self.z_logvar_p  = self.generation_z(h)
        
        if inf:
            z_list = np.array(z_list).squeeze(1)
            h_list = np.array(h_list).squeeze(1)
            return y.squeeze(1).detach().cpu().numpy(), z_list, h_list
        return y#, y_std
