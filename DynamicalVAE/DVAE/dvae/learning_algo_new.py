...
class LearningAlgorithm():
    ...
    def train(self):
        ...
        if not self.params['reload']:
            ...
            kl_warm = int(start_epoch/10)*0.2 if int(start_epoch/10)*0.2 < 1 else 1
            
            print('kl_warm is',kl_warm)
        ...
        for epoch in range(start_epoch+1, epochs):
            ...
            for num_batch, (ids, batch_data) in enumerate(self.train_dataloader):
                ...
                loss_recon = loss_MPJPE(batch_data, recon_batch_data)
            ...
            # Validation
            for num_batch, (ids,batch_data) in enumerate(self.val_dataloader):
            ...
            logger.info('Train => tot: {:.4f} recon {:.6f} KL {:.6f} Val => tot: {:.4f} recon {:.6f} KL {:.6f}'.format(train_loss[epoch], train_recon[epoch], train_kl[epoch], val_loss[epoch], val_recon[epoch], val_kl[epoch]))
